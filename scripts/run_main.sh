...
# Optional:
# Set REPO_ROOT before running if your repository is not in the default location.
# Example:
#   export REPO_ROOT="$HOME/tl-ppo-edge-cloud-federated-learning"
REPO_ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"

# Optional:
# If you want activation inside the script, set:
#   export VENV_ACTIVATE="/full/path/to/venv/bin/activate"
# Otherwise activate your environment manually before running.
VENV_ACTIVATE="${VENV_ACTIVATE:-}"

OUTDIR="${OUTDIR:-$REPO_ROOT/outputs/N10_300r_main}"
LOGFILE="$OUTDIR/run_N10_300r_main.log"
...
nohup python3 -u src/afrl_idea1_experiment_all.py \
  ...
  --strategies fedavg,heuristic,dqn,ddqn,tl_ppo,tr_dp_afrl \
  --K 8 \
  --min_participants 5 \
  --outdir "$OUTDIR" \
  --variant_label N10 \
